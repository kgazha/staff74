...
def detect_image(filename, template_filename):
    ...
    # result = Image.fromarray((resized * 255).astype(np.uint8))
    # result.save('temp.jpg')
    img_gray =resized
    # stream = open('temp.jpg', "rb")
    # bytes = bytearray(stream.read())
    # numpyarray = np.asarray(bytes, dtype=np.uint8)
    # img_gray = cv2.imdecode(numpyarray, cv2.IMREAD_UNCHANGED)
...
if __name__ == '__main__':
    ...
    for f in listdir(current_dir):
        if isfile(join(current_dir, f)) and f.lower().split('.')[-1] in ['gif', 'jpe', 'jpeg', 'jpg', 'png', 'svg', 'svgz'] \
